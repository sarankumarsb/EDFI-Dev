...
namespace EdFi.Dashboards.Resources.Tests.LocalEducationAgency
{
    public abstract class ExportStudentDemographicListServiceFixtureBase : TestFixtureBase
    {
        ...
        {
            var service = new ExportStudentDemographicListService(studentInformationRepository, studentSchoolInformationRepository, studentIndicatorRepository, studentSchoolMetricInstanceSetRepository, metricInstanceRepository, rootMetricNodeResolver, schoolInformationRepository, gradeLevelUtilitiesProvider);
            actualModel = service.Get(ExportStudentDemographicListRequest.Create(suppliedLocalEducationAgencyId, suppliedDemographic));
        }
        ...
        public virtual void Should_correctly_select_students_matching_demographic()
        {
            ...
            Assert.That(actualModel.Rows.ElementAt(0).Cells.ElementAt(2).Key, Is.EqualTo("Grade Level"));
