...
{
    public class When_loading_not_normalized_state_assessment_chart : TestFixtureBase
    {
        private const int suppliedStudentUSI = 100;
        private const int suppliedSchoolId = 200;
        private const int suppliedMetricId = 1502;
        private const int suppliedMetricVariantId = 1502;
            ...
            var data = new List<StudentMetricStateAssessmentHistorical>
                                {
                                    ...
                                    new StudentMetricStateAssessmentHistorical { StudentUSI = suppliedStudentUSI, SchoolId = suppliedSchoolId, MetricId = suppliedMetricId + 3, AdministrationYear = suppliedAdministrationYear + 4, Context = suppliedContext4, NonNormalized = suppliedNonNormalized34, Normalized = suppliedNormalized34, MetricStateTypeId = null},
        ...
        [Test]
        public void Should_load_no_strip_lines()
        {
        ...
        public void Should_load_data_correctly()
        {
            ...
            Assert.That(actualModel.SeriesCollection[0].Points[0].Value, Is.EqualTo(suppliedNonNormalized11));
            Assert.That(actualModel.SeriesCollection[0].Points[1].Value, Is.EqualTo(suppliedNonNormalized21));
            Assert.That(actualModel.SeriesCollection[0].Points[2].Value, Is.EqualTo(suppliedNonNormalized31));
        ...
        public void Should_set_state_of_most_recent_data()
        {
            ...
            Assert.That(actualModel.SeriesCollection[3].Points[1].State, Is.EqualTo(MetricStateType.Good));
            Assert.That(actualModel.SeriesCollection[3].Points[2].State, Is.EqualTo(MetricStateType.None));
            Assert.That(actualModel.SeriesCollection[3].Points[3].State, Is.EqualTo(MetricStateType.Low));
            Assert.That(actualModel.SeriesCollection[3].Points[4].State, Is.EqualTo(MetricStateType.Low));
        ...
        public virtual void Should_bind_chart_data_title()
        {
            ...
            var suppliedTitle = GetMetricMetadata().Actions.Single(x => x.Title == When_loading_not_normalized_state_assessment_chart.suppliedChartTitle).DrilldownHeader;
            Assert.That(actualModel.ChartTitle, Is.EqualTo(suppliedTitle));
        }
    }

    public class When_loading_not_normalized_state_assessment_chart_with_no_data : TestFixtureBase
    {
        private const int suppliedStudentUSI = 100;
        private const int suppliedSchoolId = 200;
        ...
        private const int suppliedRootNodeId = 400;
        private const short suppliedAdministrationYear = 2005;
        private const string suppliedTitle = "HistoricalChart";
        ...
        {
            var service = new StateAssessmentNotNormalizedChartService(repository, metricNodeResolver);
            actualModel = service.Get(StateAssessmentNotNormalizedChartRequest.Create(suppliedStudentUSI, suppliedSchoolId, suppliedMetricVariantId, suppliedTitle));
        }
