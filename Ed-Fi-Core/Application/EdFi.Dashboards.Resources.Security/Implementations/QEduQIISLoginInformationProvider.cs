...
namespace EdFi.Dashboards.Resources.Security.Implementations
{
    ...
    /// </summary>
    public class QEduQIISLoginInformationProvider : IStaffInformationProvider
    {
        ...
        public QEduQIISLoginInformationProvider(IRepository<LoginDetails> LoginInfoRepo)
        {
        ...
        {
            // VIN22012016
            //var staffUSIs = _LoginInfoRepository.GetAll().Where(cond => cond.UserId == username).Select(sel => sel.USI);
        ...
        public string ResolveUsername(IAuthenticationProvider authenticationProvider, string staffUSI, int userType) // VIN22062016
        {            
            if (!_usernamesByStaffUSI.ContainsKey(staffUSI))
            {
                var staffUSIs = _LoginInfoRepository.GetAll().Where(cond => cond.USI == Convert.ToInt32(staffUSI) && cond.UserType == Convert.ToInt32(userType)).Select(sel => sel.LoginId);                
